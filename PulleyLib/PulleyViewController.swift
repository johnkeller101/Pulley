...
@objc public protocol PulleyDelegate: AnyObject {
     ...
     */
    @objc optional func drawerChangedDistanceFromBottom(
...
@objc public class PulleyPosition: NSObject {
    ...
    public init(rawValue: Int) {
        if rawValue < 0 || rawValue > 3 {
            print(
                "PulleyViewController: A raw value of \(rawValue) is not supported. You have to use one of the predefined values in PulleyPosition. Defaulting to `collapsed`."
            )
            self.rawValue = 0
        } else {
    ...
    public static func positionFor(string: String?) -> PulleyPosition {
        ...
        default:
            print(
                "PulleyViewController: Position for string '\(positionString)' not found. Available values are: collapsed, partiallyRevealed, open, and closed. Defaulting to collapsed."
            )
            return .collapsed
        }
    }
    ...
    override public func isEqual(_ object: Any?) -> Bool {
        ...
        return rawValue == position.rawValue
    }
...
/// Represents the positioning of the drawer when the `displayMode` is set to either `PulleyDisplayMode.panel` or
/// `PulleyDisplayMode.automatic`.
///
...
/// Represents the 'snap' mode for Pulley. The default is 'nearest position'. You can use
/// 'nearestPositionUnlessExceeded' to make the drawer feel lighter or heavier.
///
/// - nearestPosition: Snap to the nearest position when scroll stops
/// - nearestPositionUnlessExceeded: Snap to the nearest position when scroll stops, unless the distance is greater than
/// 'threshold', in which case advance to the next drawer position.
...
open class PulleyViewController: UIViewController, PulleyDrawerViewControllerDelegate {
    ...
    // Internal
    fileprivate let primaryContentContainer: UIView = .init()
    fileprivate let drawerContentContainer: UIView = .init()
    fileprivate let drawerShadowView: UIView = .init()
    fileprivate let drawerScrollView: PulleyPassthroughScrollView = .init()
    fileprivate let backgroundDimmingView: UIView = .init()
    ...
    /// Get the current bottom safe area for Pulley. This is a convenience accessor. Most delegate methods where you'd
    /// need it will deliver it as a parameter.
    public var bottomSafeSpace: CGFloat {
        pulleySafeAreaInsets.bottom
    }

    /// The content view controller and drawer controller can receive delegate events already. This lets another object
    /// observe the changes, if needed.
    ...
    public var visibleDrawerHeight: CGFloat {
        if drawerPosition == .closed {
            ...
            drawerScrollView.bounds.height
        }
    }
    ...
    /// The background visual effect layer for the drawer. By default this is the extraLight effect. You can change this
    /// if you want, or assign nil to remove it.
    public var drawerBackgroundVisualEffectView: UIVisualEffectView? =
        UIVisualEffectView(effect: UIBlurEffect(style: defaultBlurEffect)) {
        willSet {
            drawerBackgroundVisualEffectView?.removeFromSuperview()
        }
        didSet {
            if let drawerBackgroundVisualEffectView, isViewLoaded
            {
    ...
    /// The inset from the top safe area when the drawer is fully open. This property is only for the 'drawer'
    /// displayMode. Use panelInsets to control the top/bottom/left/right insets for the panel.
    @IBInspectable public var drawerTopInset: CGFloat = 20.0 {
        didSet {
            if oldValue != drawerTopInset, isViewLoaded
            {
                view.setNeedsLayout()
            }
        }
    ...
    /// This replaces the previous panelInsetLeft and panelInsetTop properties. Depending on what corner placement is
    /// being used, different values from this struct will apply. For example, 'topLeft' corner placement will utilize
    /// the .top, .left, and .bottom inset properties and it will ignore the .right property (use panelWidth property to
    /// specify width)
    @IBInspectable public var panelInsets: UIEdgeInsets = .init(top: 10.0, left: 10.0, bottom: 10.0, right: 10.0) {
        didSet {
            if oldValue != panelInsets, isViewLoaded
            {
                view.setNeedsLayout()
            }
        }
    ...
    /// Depending on what corner placement is being used, different values from this struct will apply. For example,
    /// 'bottomRight' corner placement will utilize the .top, .right, and .bottom inset properties and it will ignore
    /// the .left property (use compactWidth property to specify width)
    @IBInspectable public var compactInsets: UIEdgeInsets = .init(top: 8.0, left: 8.0, bottom: 10.0, right: 8.0) {
        didSet {
            if oldValue != compactInsets, isViewLoaded
            {
                view.setNeedsLayout()
            }
        }
    ...
    @IBInspectable public var compactWidth: CGFloat = 292.0 {
        didSet {
            if oldValue != compactWidth, isViewLoaded
            {
                view.setNeedsLayout()
            }
        }
    ...
    /// Note: This property is ignored if your drawerContentViewController's view.layer.mask has a custom mask applied
    /// using a CAShapeLayer.
    /// Note: Custom CAShapeLayer as your drawerContentViewController's view.layer mask will override Pulley's internal
    /// corner rounding and use that mask as the drawer mask.
    ...
    @IBInspectable public var backgroundDimmingColor: UIColor = .black {
        didSet {
            if isViewLoaded
            {
    ...
    /// The display mode for Pulley. Default is 'drawer', which preserves the previous behavior of Pulley. If you want
    /// it to adapt automatically, choose 'automatic'. The current display mode is available by using the
    /// 'currentDisplayMode' property.
    ...
    /// The Y positioning for Pulley. This property is only oberserved when `displayMode` is set to `.automatic` or
    /// `.pannel`. Default value is `.topLeft`.
    ...
    /// The Y positioning for Pulley. This property is only oberserved when `displayMode` is set to `.automatic` or
    /// `.compact`. Default value is `.bottomLeft`.
    ...
    /// This is here exclusively to support IBInspectable in Interface Builder because Interface Builder can't deal with
    /// enums. If you're doing this in code use the -initialDrawerPosition property instead. Available strings are:
    /// open, closed, partiallyRevealed, collapsed
    ...
    /// Whether the drawer's position can be changed by the user. If set to `false`, the only way to move the drawer is
    ...
    /// This setting allows you to enable/disable Pulley automatically insetting the drawer on the left/right when in
    /// 'bottomDrawer' display mode in a horizontal orientation on a device with a 'notch' or other left/right
    /// obscurement.
    ...
    /// When true, prevents safe area insets from being propagated to the drawer content view controller.
    /// This is useful when embedding SwiftUI content via UIHostingController, as UIHostingController
    /// can inflate its size in response to safe area insets.
    public var preventDrawerSafeAreaInsetPropagation: Bool = false {
        didSet {
            if isViewLoaded {
                view.setNeedsLayout()
            }
    ...
    // The feedback generator to use for drawer positon changes. Note: This is 'Any' to preserve iOS 9 compatibilty.
    // Assign a UIFeedbackGenerator to this property. Anything else will be ignored.
    ...
    /// Get the current drawer distance. This value is equivalent in nature to the one delivered by PulleyDelegate's
    /// `drawerChangedDistanceFromBottom` callback.
    ...
    fileprivate var supportedPositions: [PulleyPosition] = PulleyPosition.all {
        didSet {
            ...
            guard !supportedPositions.isEmpty else {
            ...
            } else if currentDisplayMode == .compact, drawerPosition == .partiallyRevealed, supportedPositions
                .contains(.open)
            {
    ...
    /// The currently rendered display mode for Pulley. This will match displayMode unless you have it set to
    /// 'automatic'. This will provide the 'actual' display mode (never automatic).
     ...
     */
    public required init(contentViewController: UIViewController, drawerViewController: UIViewController) {
    ...
    override open func loadView() {
        ...
        if let drawerBackgroundVisualEffectView
        {
    ...
    override open func viewDidLayoutSubviews() {
        ...
        // Make sure our view controller views are subviews of the right view (Resolves #21 issue with changing the
        ...
        if let drawer = drawerContentViewController
        {
            ...
            // Optionally prevent safe area insets from propagating to drawer content
            // This is useful for SwiftUI content which can inflate its size based on safe areas
            if preventDrawerSafeAreaInsetPropagation {
                // Counter the safe area insets by setting negative additional insets
                let currentSafeArea = drawer.view.safeAreaInsets
                drawer.additionalSafeAreaInsets = UIEdgeInsets(
                    top: -currentSafeArea.top,
                    left: -currentSafeArea.left,
                    bottom: -currentSafeArea.bottom,
                    right: -currentSafeArea.right
                )
            }
        }
        ...
        let displayModeForCurrentLayout: PulleyDisplayMode = displayMode != .automatic ? displayMode :
        ...
        if displayModeForCurrentLayout == .drawer
        {
            ...
            if #available(iOS 11, *) {
                ...
                drawerScrollView.contentInset = UIEdgeInsets(
                    top: 0,
                    left: 0,
                    bottom: bottomLayoutGuide.length,
                    right: 0
                )
                drawerScrollView.scrollIndicatorInsets = UIEdgeInsets(
                    top: 0,
                    left: 0,
                    bottom: bottomLayoutGuide.length,
                    right: 0
            ...
            } else
            {
                ...
                var topPos = view.bounds.height - adjustedTopInset
                #if targetEnvironment(macCatalyst)
                    topPos = roundToEven(topPos)
                #endif
                drawerScrollView.frame = CGRect(
            ...
            drawerScrollView.contentSize = CGSize(
                width: drawerScrollView.bounds.width,
                height: (drawerScrollView.bounds.height - lowestStop) + drawerScrollView.bounds
                    .height - safeAreaBottomInset + (bounceOverflowMargin - 5.0)
            )

            // Update rounding mask and shadows
            let borderPath = drawerMaskingPath(byRoundingCorners: [.topLeft, .topRight, .bottomLeft, .bottomRight])
                .cgPath
        ...
        } else
        {
            ...
            if #available(iOS 11, *) {
                ...
                automaticallyAdjustsScrollViewInsets = false
                drawerScrollView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 0.0, right: 0)
                drawerScrollView.scrollIndicatorInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0.0, right: 0)
            }
            ...
            if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate
            {
                collapsedHeight = drawerVCCompliant
                    .collapsedDrawerHeight?(bottomSafeArea: safeAreaBottomInset) ?? kPulleyDefaultCollapsedHeight
                partialRevealHeight = drawerVCCompliant
                    .partialRevealDrawerHeight?(bottomSafeArea: safeAreaBottomInset) ??
            ...
            {
                lowestStop = [
                    view.bounds.size.height - compactInsets.bottom - safeAreaTopInset,
                    collapsedHeight,
                    partialRevealHeight
                ].min() ?? 0
                xOrigin = (compactCornerPlacement == .bottomLeft) ? (safeAreaLeftInset + compactInsets.left) :
                    (view.bounds.maxX - (safeAreaRightInset + compactInsets.right) - compactWidth)

                yOrigin = (compactInsets.top + safeAreaTopInset)
            } else
            {
                lowestStop = [
                    view.bounds.size.height - panelInsets.bottom - safeAreaTopInset,
                    collapsedHeight,
                    partialRevealHeight
                ].min() ?? 0
                xOrigin = (panelCornerPlacement == .bottomLeft || panelCornerPlacement == .topLeft) ?
                    (safeAreaLeftInset + panelInsets.left) :
                    (view.bounds.maxX - (safeAreaRightInset + panelInsets.right) - panelWidth)

                yOrigin = (panelCornerPlacement == .bottomLeft || panelCornerPlacement == .bottomRight) ?
            ...
            {
                // Layout scrollview
                let adjustedTopInset: CGFloat = supportedPositions
                    .contains(.partiallyRevealed) ? partialRevealHeight : collapsedHeight
            ...
            drawerScrollView.contentSize = CGSize(
                width: drawerScrollView.bounds.width,
                height: view.bounds.height + (view.bounds.height - lowestStop)
            )
    ...
    func getStopList() -> [CGFloat] {
        ...
        {
            collapsedHeight = drawerVCCompliant
                .collapsedDrawerHeight?(bottomSafeArea: pulleySafeAreaInsets.bottom) ?? kPulleyDefaultCollapsedHeight
            partialRevealHeight = drawerVCCompliant
        ...
        if supportedPositions.contains(.open)
        {
            var value = (view.bounds.size.height - drawerTopInset - pulleySafeAreaInsets.top)
            #if targetEnvironment(macCatalyst)
    ...
    private func drawerMaskingPath(byRoundingCorners corners: UIRectCorner) -> UIBezierPath {
        // In lue of drawerContentViewController.view.layoutIfNeeded() whenever this function is called, if the
        // viewController is loaded setNeedsLayout
        if drawerContentViewController.isViewLoaded {
            drawerContentViewController.view.setNeedsLayout()
        }

        let path = if let customPath = (drawerContentViewController.view.layer.mask as? CAShapeLayer)?.path {
            UIBezierPath(cgPath: customPath)
        } else {
            UIBezierPath(
    ...
    private func maskBackgroundDimmingView() {
        ...
        if let frame = drawerContentContainer.superview?.convert(drawerContentContainer.frame, to: view) {
    ...
    open func triggerFeedbackGenerator() {
        if #available(iOS 10.0, *) {
            // prepareFeedbackGenerator() is also added to scrollViewWillEndDragging to improve time between haptic
            // engine triggering feedback and the call to prepare.
    ...
    /// Bounce the drawer to get user attention. Note: Only works in .drawer display mode and when the drawer is in
    /// .collapsed or .partiallyRevealed position.
    ///
    /// - Parameters:
    ...
    public func bounceDrawer(bounceHeight: CGFloat = 50.0, speedMultiplier: Double = 0.75) {
        guard drawerPosition == .collapsed || drawerPosition == .partiallyRevealed else {
            print(
                "Pulley: Error: You can only bounce the drawer when it's in the collapsed or partially revealed position."
            )
            return
        }
    ...
    private func syncDrawerContentViewSizeToMatchScrollPositionForSideDisplayMode() {
        ...
        if !isAnimatingDrawerPosition || borderPath.boundingBox.height < drawerShadowView.layer.shadowPath?.boundingBox
            .height ?? 0.0
        {
            drawerShadowView.layer.shadowPath = borderPath
        }
    ...
    public func setDrawerPosition(
        ...
        let stopToMoveTo: CGFloat = switch drawerPosition {
        case .collapsed:
            collapsedHeight
        ...
        if animated, view.window != nil
        {
            ...
            UIView.animate(
                ...
                animations: { [weak self] () in
                    self?.drawerScrollView.setContentOffset(
                        CGPoint(x: 0, y: stopToMoveTo - lowestStop),
                        animated: false
                    )

                    // Move backgroundDimmingView to avoid drawer background being darkened
                    self?.backgroundDimmingView.frame = self?
                        .backgroundDimmingViewFrameForDrawerPosition(stopToMoveTo) ?? CGRect.zero

                    if let drawer = self
                    {
                        drawer.delegate?.drawerPositionDidChange?(
                            drawer: drawer,
     ...
     */
    public func setPrimaryContentViewController(
    ...
    public func setNeedsSupportedDrawerPositionsUpdate()
    {
        if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate
        {
            if let setSupportedDrawerPositions = drawerVCCompliant.supportedDrawerPositions?() {
                supportedPositions = currentDisplayMode == .compact ? setSupportedDrawerPositions
                    .filter(PulleyPosition.compact.contains) : setSupportedDrawerPositions
            } else {
                supportedPositions = currentDisplayMode == .compact ? PulleyPosition.compact : PulleyPosition.all
            }
        } else
    ...
    open func supportedDrawerPositions() -> [PulleyPosition] {
        if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate,
           let supportedPositions = drawerVCCompliant.supportedDrawerPositions?() {
            currentDisplayMode == .compact ? supportedPositions
                .filter(PulleyPosition.compact.contains) : supportedPositions
        } else {
            currentDisplayMode == .compact ? PulleyPosition.compact : PulleyPosition.all
        }
    }
    ...
    open func drawerChangedDistanceFromBottom(
        drawer: PulleyViewController,
        distance: CGFloat,
        bottomSafeArea: CGFloat
    ) {
        if let drawerVCCompliant = drawerContentViewController as? PulleyDrawerViewControllerDelegate {
            drawerVCCompliant.drawerChangedDistanceFromBottom?(
                drawer: drawer,
                distance: distance,
...
extension PulleyViewController: UIScrollViewDelegate {
    public func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate _: Bool) {
        if scrollView == drawerScrollView
        {
            ...
            {
                collapsedHeight = drawerVCCompliant
                    .collapsedDrawerHeight?(bottomSafeArea: pulleySafeAreaInsets.bottom) ??
            ...
            if abs(Float(currentClosestStop - heightOfOpenDrawer)) <= Float.ulpOfOne, supportedPositions.contains(.open)
            {
                closestValidDrawerPosition = .open
                      ...
                      supportedPositions.contains(.collapsed)
            {
            ...
            let snapModeToUse: PulleySnapMode = closestValidDrawerPosition == drawerPosition ? snapMode :
                .nearestPosition
            ...
            case let .nearestPositionUnlessExceeded(threshold):
                ...
                if abs(distance) > threshold
                {
                    if distance < 0
                    {
                        let orderedSupportedDrawerPositions = supportedPositions
                            .sorted(by: { $0.rawValue < $1.rawValue }).filter { $0 != .closed }
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == drawerScrollView
        {
               ...
               supportedPositions.contains(.open)
            {
                ...
                let progress: CGFloat = if fullRevealHeight == partialRevealHeight {
                    1.0
                } else {
                ...
                delegate?.makeUIAdjustmentsForFullscreen?(
                    progress: progress,
                    bottomSafeArea: pulleySafeAreaInsets.bottom
                )
                (drawerContentViewController as? PulleyDrawerViewControllerDelegate)?.makeUIAdjustmentsForFullscreen?(
                    progress: progress,
                    bottomSafeArea: pulleySafeAreaInsets.bottom
                )
                (primaryContentViewController as? PulleyPrimaryContentControllerDelegate)?
                    .makeUIAdjustmentsForFullscreen?(
                        progress: progress,
                        bottomSafeArea: pulleySafeAreaInsets.bottom
                    )
            ...
            delegate?.drawerChangedDistanceFromBottom?(
                drawer: self,
                distance: scrollView.contentOffset.y + lowestStop,
                bottomSafeArea: pulleySafeAreaInsets.bottom
            )
            (drawerContentViewController as? PulleyDrawerViewControllerDelegate)?.drawerChangedDistanceFromBottom?(
                drawer: self,
            ...
            backgroundDimmingView
                .frame = backgroundDimmingViewFrameForDrawerPosition(scrollView.contentOffset.y + lowestStop)
